...
class VI_WOO_PRODUCT_VARIATIONS_SWATCHES_Frontend_Frontend {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_dropdown_variation_attribute_options_html', array( $this, 'variation_attribute_options_html' ), PHP_INT_MAX, 2 );
		add_filter( 'woocommerce_ajax_variation_threshold', array( $this, 'viwpvs_ajax_variation_threshold' ), PHP_INT_MAX, 2 );
	}
	...
	public function variation_attribute_options_html( $html, $args ) {
		...
		if ( $is_taxonomy ) {
			...
			if ( ! $vi_attribute_type ) {
				...
				if ( ! in_array( $vi_attribute_type, array( 'button', 'color', 'image', 'variation_img', 'radio','viwpvs_default' ) ) && ! isset( $settings->get_params( 'taxonomy_profiles' )[ $attribute ] ) ) {
		...
		$new_html .= 'data-display_type="'.$display_type_class.'" data-show_selected_item="'.$attribute_attr_selected.'" ';
		$new_html .= '>';
					...
					case 'image':
						foreach ( $terms as $term ) {
                            ...
                            <div class="<?php echo esc_attr( $term_class ); ?>"
                                 data-attribute_label="<?php echo esc_attr( $term_name ); ?>"
                                 data-attribute_value="<?php echo esc_attr( $term->slug ); ?>">
                                <img src="<?php echo esc_url( $img_url ); ?>" alt="<?php echo esc_attr( $term->slug ); ?>" data-loop_src="<?php echo esc_url( $img_loop_src ); ?>"
